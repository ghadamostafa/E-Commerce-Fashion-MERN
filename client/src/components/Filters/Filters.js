...
const Filters = ({ getProducts, pageNumber }) => {
    ...
    useEffect(() => {
            ...
            url += params ? `${params}&page=${pageNumber}`
    ...
    const toggleSubmenu = () => {
        submenuToggle === 'hide' ? setSubmenuToggle('show') : setSubmenuToggle('hide')
    }
    ...
    return (
        <section >
            {/* categories filter */}
            <h6 className="text-uppercase font-weight-bold mb-3 filter-title-color">Categories</h6>
            {categories.map((category) => {
                return <div className="mt-2 mb-2 pl-2" key={category._id}>
            ...
            <section>
                <h6 className="text-uppercase font-weight-bold mb-3 filter-title-color">Tags</h6>
                <div>
                    {tags.map((tag) => {
                        return <button key={tag._id} className="btn btn-outline-secondary mr-2 mt-2"
                            onClick={() => setTagFilter(tag.slug)}>{tag.name}</button>
                    })}
                </div>
            </section>
        </section >
